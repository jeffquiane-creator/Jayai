...
def brokerage_ui():
    ...
    with left:
        ...
        compare_multi = st.multiselect("Compare with… (optional)", [b for b in brokerages if b != picked])
    with right:
        pass
...
"
        ...
        file_name=f"{picked.replace(' ','_')}_pack.txt", mime="text/plain"
    )
    ...
    if compare_multi:
        st.markdown("---")
        st.markdown("### Compare")
        comp = bdf[bdf["brokerage"].isin([picked] + compare_multi)].copy()
        comp = comp[["brokerage","one_liner","rebuttal","sms"]]
        comp = comp.rename(columns={"brokerage":"Brokerage","one_liner":"One‑liner","Rebuttal":"Rebuttal","sms":"SMS"})
        comp = comp.rename(columns={"rebuttal":"Rebuttal"})  # ensure correct name
