...
def load_dataset(path: str) -> List[QAItem]:
    ...
    with open(path, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            # Strip whitespace from keys to handle accidental spaces in
            # the header.  Use a new dict for clarity.
            ...
            if not (filename and question and category is not None and answer is not None):
                # It's OK to silently skip invalid rows; alternatively, you
                # could log to st.error or print to console for debugging.
                continue
            qa_list.append(
                QAItem(
                    filename=filename,
