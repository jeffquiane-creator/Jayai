...
def load_dataset(path: str) -> List[QAItem]:
    ...
    with open(path, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            # Skip rows missing mandatory fields.  Category and answer can be empty strings,
            # so we explicitly check None.
            if filename is None or question is None or category is None or answer is None:
                continue
            qa_list.append(
                QAItem(
                    filename=filename,
