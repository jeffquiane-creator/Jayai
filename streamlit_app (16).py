...
def load_brokerage_table() -> Tuple[pd.DataFrame, str]:
    ...
    b_col = pick(list(raw.columns), BROKER_ALIASES["brokerage"])
    r_col = pick(list(raw.columns), BROKER_ALIASES["rebuttal"])
    s_col = pick(list(raw.columns), BROKER_ALIASES["sms"])
    o_col = pick(list(raw.columns), BROKER_ALIASES["one_liner"])
    n_col = pick(list(raw.columns), BROKER_ALIASES["notes"])
...
def brokerage_ui():
    ...
    if main_row.get("notes", ""):
        st.markdown("> " + main_row["notes"])

    st.markdown("**Quick actions**")
    ac = st.columns(3)
    with ac[0]:
        copy_button("Copy Rebuttal", main_row.get("rebuttal",""), key=f"copy-breb-{main_row['bid']}")
    with ac[1]:
        copy_button("Copy SMS", main_row.get("sms",""), key=f"copy-bsms-{main_row['bid']}")
    with ac[2]:
        copy_button("Copy One‑liner", main_row.get("one_liner",""), key=f"copy-bone-{main_row['bid']}")
    ...
    # Build the pack text safely via join (no unterminated string risk)
    pack_lines = [
        f"Brokerage: {picked}",
        "",
        "Rebuttal:",
...
st.download_button(
    ...
    # Compare to Funnel Pilot only (optional)
    if compare_fp:
        st.markdown("---")
        ...
        if fp.empty:
            ...
            with colA:
                ...
                st.write(f"**Brokerage:** {picked}")
                st.markdown("**One‑liner**")
                st.write(one_liner_txt or "_(none)_")
                st.markdown("**Rebuttal**")
                st.markdown(rebuttal_txt or "_(none)_")
                st.markdown("**SMS**")
                st.code(sms_txt or "(none)", language="text")
            with colB:
            ...
            # Positioning Script (safe)
            st.markdown("#### Positioning Script")
            script = (
                "Totally hear the appeal of {picked}. Here’s why agents who compare it to Funnel Pilot usually choose FP:
