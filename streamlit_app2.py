# streamlit_app.py — Funnel Pilot Hub (all features, browser TTS)
# No pyttsx3/gTTS needed. Audio plays in the browser (Web Speech API).
# Auto-detects your files:
#   Agent:  Agent_Objections_Rebuttals.xlsx/.csv, Objection_Rebuttal_Master_500 (1).csv
#   Broker: Top_25_Brokerage_Rebuttals_Final_with_Power_Statements.xlsx (or CSV),
#           Top_25_Brokerage_Rebuttals_FunnelPilot.xlsx (or CSV)
    ...
    <script>
      ...
      if (btn_{btn_id} && navigator.clipboard) {{
        btn_{btn_id}.onclick = async () => {{
          ...
          catch (e) {{ alert('Copy blocked by your browser; select and copy manually.'); }}
        }};
      }}
    </script>
    """
...
def discover_dataset(expected_cols, preferred_names, min_required):
    """Find first CSV/XLSX that contains the required cols."""
    cwd = set(glob.glob("*.xlsx") + glob.glob("*.csv"))
    ...
    for p in candidates:
        ...
        if all(mr.lower() in lower for mr in min_required):
            return ensure_cols(df, expected_cols), p, scanned
    return pd.DataFrame(columns=expected_cols), None, scanned
...
with st.sidebar:
    ...
    st.caption("Applies to every ▶️ Play button in the app.")

# ---------- Initial auto-detect (once) ----------
if "agent_df" not in st.session_state:
    adf, apath, aseens = discover_dataset(
        AGENT_COLS,
        preferred_names=[
    ...
    st.session_state.agent_df   = adf
    st.session_state.agent_path = apath
    st.session_state.agent_seen = aseens

if "brokerage_df" not in st.session_state:
    bdf, bpath, bseens = discover_dataset(
        BROKERAGE_COLS,
        preferred_names=[
    ...
    st.session_state.brokerage_df   = bdf
    st.session_state.brokerage_path = bpath
    st.session_state.brokerage_seen = bseens
...
with st.expander("📦 Data Sources • Upload / Export"):
    ...
    with c1:
        ...
        st.write("Loaded file:", st.session_state.get("brokerage_path") or "— not detected —")
    ...
    with c2:
        ...
        st.write("Loaded file:", st.session_state.get("agent_path") or "— not detected —")
...
if mode == "🙋 Agent Objections":
    ...
    if adf.empty:
        st.warning("No agent objection data found. Upload/select your dataset above.")
    else:
        ...
        with right:
            if selected_objection is not None:
                ...
                # Optional local audio file (if the row has a path)
...
# ---------- Brokerage Comparison ----------
    ...
    if bdf.empty:
        st.warning("No brokerage dataset found. Upload/select your dataset above.")
    else:
        left, right = st.columns([2, 3])
